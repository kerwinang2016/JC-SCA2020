			...
			else
			{
				if (this.model.isSelectionComplete() && this.isQuotable() && this.validateLogin())
				{
					this.application.ProductListModule.Utils.getRequestAQuoteProductList().done(function (product_list_json)
					{
						...
						{
							var item_present_in_list = _.find(product_list_json.items, function(itemQuote)
							{
								var flattenOption = [];
								if(!!self.model.getSelectedOptions().length)
								{
									_.each(itemQuote.options, function(option)
									{
										if(option.value)
										{
											flattenOption.push(option.value);
										}
									});
								}

								return (
									self.model.get('item').id === parseInt(itemQuote.item.internalid) &&
									_.isEqual(self.model.getSelectedOptions(), flattenOption)
								);
							});
