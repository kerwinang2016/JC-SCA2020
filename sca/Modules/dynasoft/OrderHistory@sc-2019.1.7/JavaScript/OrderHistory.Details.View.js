	...
	)
{
	...
	,	getBreadcrumbPages: function ()
		{
			...
			return [
				{
						text: _('Order History').translate()  //18/07/2019 Saad
					,	href: '/purchases'
				}
			, 	{
						text: '#' + this.model.get('tranid')
					,	href :'/purchases/view/' + this.model.get('recordtype') + '/' + this.model.get('internalid1')
				}
			];
		}
	...
	,	addToCart: function (e)
		{
			...
			,	quantity = 1 //this.$(e.target).data('partial-quantity') || this.$(e.target).data('item-quantity')
			...
			if(selected_line.get('quantity')){
				selected_line.set('quantity', 1);
			}

			var dateNeeded = '1/1/1900';
			for(var i = 0; i < selected_line.get('options').models.length; i++){
				if(selected_line.get('options').models[i].attributes.cartOptionId == 'custcol_avt_date_needed'){
					selected_line.get('options').models[i].attributes.value.internalid = dateNeeded;
				}

				if(selected_line.get('options').models[i].attributes.cartOptionId == 'custcol_avt_modified_date_needed'){
					selected_line.get('options').models[i].attributes.value.internalid = dateNeeded;
				}

				if (selected_line.get('options').models[i].attributes.cartOptionId  == 'custcol_avt_wbs_copy_key') {
					var selectedItemInternalId = selected_line.get('item').get('internalid');
					var itemUniqueCopy = selectedItemInternalId + '_' + new Date().getTime();
					selected_line.get('options').models[i].attributes.value.internalid = itemUniqueCopy;
				}

				if (selected_line.get('options').models[i].attributes.cartOptionId  == 'custcol_site_cogs') {
					selected_line.get('options').models[i].attributes.value.internalid = '<div></div>';
				}

				// if(selected_line.get('options').models[i].attributes.cartOptionId == 'custcol_expected_delivery_date'){
				// 	selected_line.get('options').models[i].attributes.value.internalid = '';
				// }
			}
			...
			LiveOrderModel.getInstance().addLine(selected_line).done(function ()
			{
				...
				{
					message = quantity > 1 ?
					_('$(0) Items were added to <a href="#" data-touchpoint="viewcart">your cart</a><br/>').translate(quantity) : // zain 30-08-19
					_('Item was added to <a href="#" data-touchpoint="viewcart">your cart</a><br/>').translate(); // zain 30-08-19
				}
	...
	,	childViews: {
		...
		,	'NonShippableLines': function ()        // 26/08/2019 Saad
			{
				var cmtItemArr = [9900, 9899, 9902, 9901, 9898, 9904, 9903, 292321, 292329, 292326, 292327, 292328, 292332, 292330, 292331]; //these items will not show on sale order
				var collection = _.filter(this.non_shippable_lines, function(line){
					var itemId = line.get('item').get('internalid');
					if(cmtItemArr.indexOf(itemId) == -1){
						return line;
					}
				})

				return new BackboneCollectionView({
					collection: collection || this.getNonShippableLines()
	...
	,	getContext: function()
		{
			var createdBy = '';               //26/08/2019 Saad
			if(this.model.get('fulfillments').models[0]){          // 26/08/2019 Saad
				createdBy = this.model.get('fulfillments').models[0].createdby;
			}
			createdBy = createdBy ? createdBy : this.model.get('custbody_so_created_by'); //12/11/2019 Umar Zulfiqar
			...
			var pdfUrl = "https://" + window.location.hostname + "/app/site/hosting/scriptlet.nl?script=121&deploy=1&compid=3857857&h=a10878b386852988f44f&custparam_record_id=" + this.model.get('internalid') + "&custparam_record_type=salesorder&custparam_template=5";
			...
			return {
				...
				,	pdfUrl: pdfUrl
				,	createdBy: createdBy ? createdBy : ' '      //   26/08/2019 Saad
			};
		}
