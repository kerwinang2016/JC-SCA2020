		...
		,	setOptionsArray: function (options, dont_validate)
			{
				...
				{
					self.setOption(option.attributes.cartOptionId, { 
						internalid: option.attributes.value.internalid
					,	label: option.attributes.value.label
					}, dont_validate);
				});
		...
		,	initialize: function initialize ()
			{
				...
				var self = this;
				this.itemOptions = {};
				this.set('options', this.getPosibleOptions());
				this.set('vendorDetails', '');
				this.set('extraQuantity', '');
				this.set('vendorPickList', '');
		...
		,	getPosibleOptionByCartOptionId: function (cart_option_id)
			{
				...
				for(var i = 0; i < optionModels.length; i++){
					if(optionModels[i].attributes.cartOptionId == cart_option_id){
						return optionModels[i].attributes;
					}
