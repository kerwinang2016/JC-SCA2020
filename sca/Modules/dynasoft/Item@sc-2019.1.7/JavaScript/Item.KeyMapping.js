	...
	function getKeyMapping (configuration)
	{
		...
		*/
		...
		,	_breadcrumb: function (item)
			{
				...
				if (hash.indexOf('?') != -1) {
					...
					if (tempDetail) {
						...
						if(hash.indexOf('CMT-Item')!=-1 && hash.indexOf('&product=')!=-1)
						{
							if(hash.indexOf('keywords')!=-1)
							{
								productName = tempDetail[2].indexOf("=") != -1 ? tempDetail[2].split('=')[1] : "";
							}
							else
							{
								productName = tempDetail[1].indexOf("=") != -1 ? tempDetail[1].split('=')[1] : "";
							}
						}
						else if(hash.indexOf('|item')!=-1 && hash.indexOf('&product=')!=-1) //20/01/2020
						{
							if(hash.indexOf('|item')!=-1 && hash.indexOf('keywords=')!=-1)
							{
								productName = tempDetail[2].indexOf("=") != -1 ? tempDetail[2].split('=')[1] : "";
							}
							else{
						...
						{	
							productName = (tempDetail[2]!=undefined && tempDetail[2].indexOf("=") != -1) ? tempDetail[2].split('=')[1] : "";   //16/01/2020
						}
						if(clientId){
							breadcrumb.push(
							{
								href: "/item-type?client=" + clientId
							,	text: "Item Type"
							});
						...
						if(clientId && productName){
							breadcrumb.push(
							{
								href: "/item-type/" + productName + "?client=" + clientId
								,text: productName
							});
		...
		,   _ratingsCount: function (item)
			...
			// @property {object} _ratingsCount How many times this item was reviewed
		,   _vendorName: function (item)
			{
				return item.get('custitem_vendor_name') || '';
			}
