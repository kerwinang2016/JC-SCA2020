		...
		,	put: function()
			{
				...
				{
					var isDateAppliedOnAllLines = false;
					var dateNeeded = '';
					...
					if(isDateAppliedOnAllLines) {
						for(var i = 0; i < lines.length; i++){
							...
							for(var j =0; j < lines[i].options.length; j++){
								if(lines[i].options[j].cartOptionId == 'custcol_avt_date_needed'){
									lines[i].options[j].value = {internalid: dateNeeded , label: dateNeeded }
								}

								if(lines[i].options[j].cartOptionId == 'custcol_avt_modified_date_needed'){
									lines[i].options[j].value = {internalid: '1/1/1900' , label: '1/1/1900' }
								}
							}
