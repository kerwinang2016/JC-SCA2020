	...
	)
{
	...
	,	addLines: function addLines (lines)
		{
			...
			.then(function(){
				...
				,	promise = lines.sync('create', lines, {killerId: AjaxRequestsKiller.getKillerId(), validate: false});
	...
	,	addProduct: function addProduct (product)
		...
		//@return {jQuery.Deferred}
	,	addProducts: function addProducts (products, isProductListModule)
		{
			var lines = _.map(products, function (product)
			{
				return LiveOrderLineModel.createFromProduct(product, null, null, isProductListModule);
			});
	...
	,	addItems: function (items, options)
		{
			...
			var lines = _.map(items, function (item)
			{
				...
				,	options: options //_.values(line_options).length ? line_options : null
				};
			});

			//lines.attribute.options = line_options;
