	...
	)
{
	...
	,	isMultiSite: false//ModelsInit.context.getFeature('MULTISITE')
	...
	,	getLines: function ()
		{
			...
			for (var i = 1; i <= this.record.getLineItemCount('item'); i++)
			{
				...
				else
				{
					...
					,	description: this.record.getLineItemValue('item', 'description', i)
	...
	,	loadItemsWithSuiteScript: function (items_to_query)
		{
			...
			if (items_to_query.length > 0)
			{
				...
				,	columns = [
					...
					,	new nlobjSearchColumn('description', 'item') //11/09/2019 Saad saad
					]
				...
				_.each(inactive_items_search, function (item)
				{
					...
					,	description: item.getValue('description', 'item')	//11/09/2019 Saad saad
