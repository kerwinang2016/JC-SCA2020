			...
			_buildColumns: function (selectedColumns, transaction) {
				function createColumn(transaction, column) {
					switch (column.id) {
						...
						case 'custbody_customer_name':
							return {
								label: _(column.label).translate()
								, type: column.type
								, name: column.name
								, value: transaction.get('custbody_customer_name') ? transaction.get('custbody_customer_name') : 'Multiple'
							}
						case 'customer_name_type':
							return {
								label: _(column.label).translate()
								, type: column.type
								, name: column.name
								, value: transaction.get('custbody_customer_name') ? 'Production' : 'Shipping'
							}
						case 'createdfrom':  //11/09/2019 Saad saad
							return {
								label: _(column.label).translate()
								, type: column.type
								, name: column.name
								, value: transaction.get('createdFromSaleorder')
							}	
						default:
							var init = {
								label: _(column.label).translate()
