...
define('FormRenderer.View',  ['Client.Model', 'Profile.Model'], function (ClientModel, ProfileModel)
{
	...
	,	render: function(){
			var type = this.options.mode.split("|")[1],
				...
				parent = this.profile_model.get("parent");
			var	html = ""
			var	Model = "";
			...
			switch(type){
				...
				// 	var Model = null;
				// 	if(id == "new"){
				// 		profile_model = new ClientModel();
				// 	} else {
				// 		profile_model = ProfileModel.client_collection.find(function(data){
				// 			return data.get("internalid") == id;
				// 		});
				// 	}
				// 	html = clientForm(profile_model, parent);
				// 	break;
			}
