...
, 	[	'SC.Configuration'
	...
	return Backbone.View.extend({
		...
		events: {
			...
			'click [id=search-item-keyword]': 'searchItemKeyword'
		}

		,
		initialize: function (options) {		//15/01/2020
				...
				//15/01/2020
				this.keywordsFound = false;    //15/01/2020
				var urlParams = window.location.search || window.location.hash;
				_.each(urlParams.split('&'), function (v) {
					if (v.indexOf('keywords') != -1 /*&& !_.isEmpty(v.split('=')[1])*/ ) {
		...
		,	searchItemKeyword: function () //15/01/2020
		{
			...
			if (historyFragment.indexOf("&keywords=") != -1) {
				historyFragment = historyFragment.split('&keywords=')[0]
			}
			var searchKey = $("#item-search-key-value").val();
			var searchKey1 = $("#item-search-key-value1").val();

			var navigateUrl = historyFragment + '&keywords=' + searchKey1;

			if (historyFragment.indexOf('?') == -1) //09/01/2020
			{
				navigateUrl = historyFragment + '?keywords=' + searchKey;
			}

			else if(historyFragment.indexOf("?keywords=") != -1 && historyFragment.indexOf("&keywords=")== -1) //04/02/2020
			{
				navigateUrl = historyFragment + '&keywords=' + searchKey;
			}
		...
		_gotoShow: function gotoShow(e) {
				var original = this.translator.getUrl(),
					default_value = _.find(this.resultsPerPage, function (result) {
						return result.isDefault;
					}).id,
		...
		_gotoOrder: function gotoOrder(e) {
				var original = this.translator.getUrl(),
					default_value = _.find(this.sortOptions, function (result) {
						return result.isDefault;
					}).id,
		...
		getBreadcrumbPages: function () {
				...
				if (this.model.get('category')) {
					...
					if (hash!= 'applications' && hash.indexOf('Inventory/Fabrics') == -1 && hash.indexOf('Inventory/Inventory-Accessories') == -1 && hash.indexOf('Inventory/Packaging') == -1) {
						...
						} else if (hash.indexOf('item-type?') != -1) {
							paramarr = hash.split('item-type?')[1];
						} else {
							paramarr = hash.split('item-type?')[1];
						}
						client = paramarr.split('?')[1];
					}

					_.each(list, function (bread) {
						breadcrumb.push({
							'text': bread.name,
							'href': bread.fullurl + '?' + client,
					...
					breadcrumb.push({ //27/08/2019 Saad
						href: '#',
		...
		childViews: {
			...
			'Facets.Items': function () {  //15/01/2020
				...
				var urlsearch = window.location.href; //07/01/2020 search keyword
				if (urlsearch.indexOf('search') == -1) {
					if (!this.keywordsFound && (urlsearch.indexOf('custitem_vendor_name') == -1 && urlsearch.indexOf('custitem_fabric_color') == -1)) { //17/12/2019 saad
						items = [];
						this.model.set('total', 0);
					}
					else if(urlsearch.indexOf('custitem_vendor_name') != -1 || urlsearch.indexOf('custitem_fabric_color') != -1)
					{
						items = this.model.get('items');
						this.model.set('total', this.model.get('items').length);
					}
				} else {
					items = this.model.get('items');
					this.model.set('total', this.model.get('items').length);
			...
			'Facets.Browse.CategoryHeading': function () {
				...
				,
			'Facets.CategoryCellsMen': function () //03-12-2019 saad
			{
				var categories = this.model.get('category').get('categories');
				var menCollection = [],
					womenCollection = [];
				_.each(categories, function (v) {
					if (!(v.name.includes("Ladies"))) {
						menCollection.push(v);
					} else {
						womenCollection.push(v);
					}
				});
				return new BackboneCollectionView({
			...
			,
			'Facets.CategoryCellsWomen': function () //03-12-2019 saad
			{
				var categories = this.model.get('category').get('categories');
				var menCollection = [],
					womenCollection = [];
		...
		,	client = (clientParam) ? "&client="+clientParam.split('=')[1] : "";
			...
			var cmtUrl = '',
				isCmtUrl = false;
			var hash = window.location.href.split(window.location.host + '/')[1];
			if (hash.indexOf('Inventory/Fabrics') == -1 && hash.indexOf('Inventory/Inventory-Accessories') == -1 && hash.indexOf('Inventory/Packaging') == -1 && hash.indexOf('item-type/') != -1) {
				var paramarr = hash.split('item-type/')[1];
				var product = paramarr.split('?')[0];
				var client = paramarr.split('?')[1];
			}
			if (client && product) {
				var cmtUrl = '/CMT-Item?' + client + '&product=' + product;
				isCmtUrl = true;
			}
