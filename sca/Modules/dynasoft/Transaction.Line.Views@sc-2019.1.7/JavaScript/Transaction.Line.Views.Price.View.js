	...
	,	getCustomPrice: function() 
		{
			...
			for(var j = 0; j < lineOptions.length; j++){
				if(lineOptions[j].attributes.cartOptionId == "custcol_order_list_line_item_total"){
					lineTotal = lineOptions[j].attributes.value.label || lineOptions[j].attributes.value.internalid
				}
			}
	...
	,	getContext: function ()
		{
			...
			var customPrice = this.getCustomPrice();
			...
			,	customPrice: (customPrice != 0) ? customPrice : false
