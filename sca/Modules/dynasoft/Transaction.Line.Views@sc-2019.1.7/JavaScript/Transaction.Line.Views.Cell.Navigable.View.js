	...
	,	getContext: function ()
		{
			...
			var description_SaleOrder = this.model.get('description');  //11/09/2019 saad
			if(description_SaleOrder)
			{
				description_SaleOrder = description_SaleOrder.replace(/<br\s*\/?>/gi,"\n");
			}
			...
			for(var j = 0; j < line.get("options").models.length; j++){
				if(line.get("options").models[j].attributes.cartOptionId == "custcol_order_list_line_item_total"){
					var tempValue = line.get("options").models[j].attributes.value.label || line.get("options").models[j].attributes.value.internalid
					if(tempValue){
						customPrices = parseFloat(tempValue);
					}
				}

				// added if to get the fabric detail to display
				if(line.get("options").models[j].attributes.cartOptionId == "custcol_custom_fabric_details"){
					fabric_details = JSON.parse(line.get("options").models[j].attributes.value.label);
				}
			}

			// added if to get the fabric detail to display
			if(item.get('itemid') == "CMT Item"){
				item.set("itemid", "CMT Item - "+ fabric_details.collection+" ("+fabric_details.code+")");
			}
			...
			return {
				...
				,	itemName: item.get('itemid')  //13/09/2019 saad
				...
				,	customPrices : customPrices ? customPrices : 0.00
				,	currencSymbol: SC.ENVIRONMENT.currentCurrency.symbol ? SC.ENVIRONMENT.currentCurrency.symbol : '$'
				,	description: description_SaleOrder //11/09/2019 Saad saad
			};
		}
