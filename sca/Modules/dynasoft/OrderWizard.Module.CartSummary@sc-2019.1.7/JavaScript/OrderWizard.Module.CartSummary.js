	...
	)
{
	...
	,	initialize: function initialize ()
		{
			...
			var suitetaxes = Configuration.get('siteSettings.isSuiteTaxEnabled');
	...
	,	getCustomPriceTotal: function(model) 
		{
			var lineTotal = 0;
			var linesModels = model.get('lines').models;
	...
	,	getContext: function getContext ()
		{
			...
			var linesTotal;
			if(typeof window.linesTotal == 'number'){
				linesTotal = window.linesTotal;
			} else{
				linesTotal = this.getCustomPriceTotal(model);
				window.linesTotal = linesTotal;
			}

			if(typeof window.itemCount == 'number'){
				item_count = window.itemCount;
			} else{
				window.itemCount = item_count;
			}
			...
			return {
				...
				,	taxLabel: !!Configuration.get('summaryTaxLabel') ? Configuration.get('summaryTaxLabel') : 'Tax Total'
					// @property {String} taxLabel
				,	suiteTaxesEnabled: !!this.wizard.model.get('suitetaxes')
					// @property {String} taxLabel
				,	suiteTaxes: this.wizard.model.get('suitetaxes')
				,	linesTotal : linesTotal ? linesTotal : '0.0'
