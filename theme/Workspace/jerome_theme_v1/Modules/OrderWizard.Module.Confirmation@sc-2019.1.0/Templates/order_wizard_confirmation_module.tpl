<div class="order-wizard-confirmation-module alert fade in">
	<h2 class="order-wizard-confirmation-module-title">{{translate 'Processed!'}}</h2> <!-- zain 26-08-19 -->
	<p class="order-wizard-confirmation-module-body" name="orderNumber">{{translate 'Your order number is'}}
		<strong>
			...
			{{else}}
				{{#if issubtailor}}
					<br/>Your Order has been placed on your main company {{parentname}}>
				{{/if}}
				#{{confirmationNumber}}
			{{/if}}
		</strong>
	</p>
	<!-- <p class="order-wizard-confirmation-module-body">{{translate 'We received your order and will process it right away.'}}</p>  --> <!-- zain 26-08-19 -->
	...
	{{/if}}
	<a class="order-wizard-confirmation-module-continue" href="{{continueURL}}" {{#if touchPoint}}data-touchpoint="home"{{/if}} data-hashtag="#/">{{translate 'Continue Ordering'}}</a> <!-- zain 26-08-19 -->
		<!-- DOWNLOAD AS PDF -->
