	...
	{{else}}
		<button class="product-list-control-button-wishlist" data-action="show-productlist-control" data-toggle="showFlyout" type="button" >
			{{translate 'Add to my product lists'}} <!--zain 18-07-19 -->
		</button>
	{{/if}}
	...
	<h5 class="product-list-control-flyout-title" {{#if hideNewProductList}}style="display: none"{{/if}}>{{translate 'Or: '}}</h5>
	<div class="product-list-control-new-product-list-container" data-type="new-item-container" {{#if hideNewProductList}}style="display: none"{{/if}}></div>
</div>
